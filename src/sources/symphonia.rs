...
pub fn start(file_path: &str) -> MixerInput {
    let (tx, rx) = mpsc::channel(128);

    let file_path = file_path.to_string();
    tokio::spawn(async move {
        loop {
            let result = play_file(file_path.clone(), tx.clone()).await;

            if let Err(e) = result {
                eprintln!("Error while playing file: {:?}", e);
            }
        }
    });
            ...
            Ok(audio_buf) => {
                ...
                if let Some(buf) = &mut sample_buf {
                    ...
                    for i in 0..samples.len() / 2 {
                        ...
                        tx.send((samples[i * 2], samples[i * 2 + 1]))
                            .await
                            .expect("Expected mixer channel to never close");
                    }
                }
            }
            Err(e) => eprintln!("Error while decoding: {:?}, skipping song", e),
        }
    }
}
